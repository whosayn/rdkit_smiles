...
static size_t get_atomic_number(std::string_view& atom_symbol) {
    static constexpr std::array<std::pair<std::string_view, size_t>, 138>
        info {{
            {"*", 0},
            {"H",  1},
            {"He", 2},
    ...
    return entry == info.end() ? std::numeric_limits<size_t>::max() : entry->second;
}
...
static void validate_atoms(std::vector<AtomInfo>& atoms) {
    for (auto& atom : atoms) {
        if (std::isdigit(atom.name[0])) {
            auto atomic_number = stoi(atom.name);
        } else if (get_atomic_number(atom.name) == std::numeric_limits<size_t>::max()) {
...
static void validate_rings(const std::vector<RingInfo>& rings) {
    ...
    for (auto& ring_info : rings) {
        if (stoi(ring_info.ring_number) == 0) {
            std::cerr << "'" << ring_info.ring_number << "'" << " is not a valid ring number" << std::endl;
            continue;
        }
        ...
        if (ring_start.bond_token.empty() || ring_info.bond_token.empty()) {
            ...
            std::cerr << "ring '" << ring_info.ring_number << "' has different closing bond types" << std::endl;
        }
